    	...
    	var selectAll=document.getElementById("selectAll");
			...
			deleteStudent.onclick=function(){
				...
				if(queren==true){
					...
					var $url="http://localhost/SSM_bookmanger/monster/"+str;
    ...
    var  outAll=document.getElementById("outAll");
			...
			outAll.onclick=function(){
				...
				if(flag){
					...
					window.location.href="http://localhost/SSM_bookmanger/outAll "
			...
			outSelect.onclick=function(){
				...
				if (flag == false) {
					...
					if (flag) {//确定
						...
						window.location.href = "http://localhost/SSM_bookmanger/outSelect/"+str;
    ...
    <div class="container">
		...
		<table class="table" align="center" width="200px" height="100px" border="1px" cellspacing="0" bordercolor="silver">
			...
			<c:forEach items="${pb.beanList}" var="s" varStatus="f">
              ...
              <!--    <td><a href="monster/${s.id }" class="deleteId btn btn-primary">删除</a></td>-->
		...
		<center>
			...
			<li><a href="monsterss?pageNow=1">首页</a> &nbsp;&nbsp;<li>
			<c:if test="${pb.pageNow>1 }">
				<li><a href="monsterss?pageNow=${pb.pageNow-1 }">上一页</a><li>
			</c:if>
			&nbsp; 
			<!-- 分两种情况：
		         如果页数小于10：
		 ...
		 <c:forEach begin="${begin }" end="${end }" var="i">
				<c:choose>
					...
					<c:otherwise>
						<li><a href="monsterss?pageNow=${i }">${i }</a></li>
					</c:otherwise>
				</c:choose>
			...
			<c:if test="${pb.pageNow<pb.pages }">
				<li><a href="monsterss?pageNow=${pb.pageNow+1 }">下一页</a></li>
			</c:if>
			&nbsp;&nbsp; <li><a href="monsterss?pageNow=${pb.pages }">尾页</a></li>
           </ul>
		</center>
		...
		<table align="center">
			<tr align="center">
				...
				<a href="http://localhost/ssm_crud/addUI">添加分类</a></td>
			</tr>
		</table>
	</form>
</body>
